...
if(!empty($_POST))
{
	...
	$sub=$_POST['sub'];
	$subb=$_POST['subb'];
	$faculty_id = uniqid();                        //generates unique id
...
<br>
    ...
    <td>Sr. No.</td>                              <!--columns or table data-->
    <td>Name</td>
    <td>Subject I</td>
    <td>Subject II</td>
    <td>Delete</td>
    ...
    <?php
	$sr=1;
	$h=mysqli_query($al,"select * from faculty");    //fetches data in same order as database
	while($j=mysqli_fetch_array($h))
	{
		?>
        <tr>                                       <!--row data-->
        <td><?php echo $sr;$sr++;?></td>
        <td><?php echo $j['name'];?></td>          <!--array indexes of fetched data-->
        <td><?php echo $j['s1'];?></td>
        <td><?php echo $j['s2'];?></td>
